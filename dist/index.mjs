...
const DrawerConfigContext = React__default.createContext(undefined);
const useDrawerConfig = ()=>{
    return React__default.useContext(DrawerConfigContext);
};
...
function preventScrollMobileSafari(container) {
    let scrollable;
    let lastY = 0;
    let onTouchStart = (e)=>{
    ...
    let restoreStyles = chain$1(setStyle(container || document.documentElement, 'paddingRight', `${window.innerWidth - document.documentElement.clientWidth}px`));
...
function useScaleBackground() {
    ...
    React__default.useEffect(()=>{
        if (isOpen && shouldScaleBackground) {
            ...
            const wrapper = (container || document).querySelector('[data-vaul-drawer-wrapper]') || (container || document).querySelector('[vaul-drawer-wrapper]');
            if (!wrapper) return;
 ...
 */ function usePositionFixed({ isOpen, modal, nested, hasBeenOpened, preventScrollRestoration, noBodyStyles, container }) {
...
function ConfigProvider({ children, container, containerId }) {
    const [resolvedContainer, setResolvedContainer] = React__default.useState(container || null);
    React__default.useEffect(()=>{
        if (container) {
    ...
    return /*#__PURE__*/ React__default.createElement(DrawerConfigContext.Provider, {
        value: {
            container: resolvedContainer
        }
    }, children);
}
function Root({ open: openProp, onOpenChange, children, onDrag: onDragProp, onRelease: onReleaseProp, snapPoints, shouldScaleBackground = false, setBackgroundColorOnScale = true, closeThreshold = CLOSE_THRESHOLD, scrollLockTimeout = SCROLL_LOCK_TIMEOUT, dismissible = true, handleOnly = false, fadeFromIndex = snapPoints && snapPoints.length - 1, activeSnapPoint: activeSnapPointProp, setActiveSnapPoint: setActiveSnapPointProp, fixed, modal = true, onClose, nested, noBodyStyles = false, direction = 'bottom', defaultOpen = false, disablePreventScroll = true, snapToSequentialPoint = false, preventScrollRestoration = false, repositionInputs = true, onAnimationEnd, container: containerProp, autoFocus = false }) {
    var _drawerRef_current, _drawerRef_current1;
    const config = useDrawerConfig();
    const container = containerProp || (config == null ? void 0 : config.container);
...
export { ConfigProvider, Content, Drawer, Handle, NestedRoot, Overlay, Portal, Root };
